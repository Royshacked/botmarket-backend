...
const ALLOWED = { chat: id => ['gpt-6-luna', 'claude-sonnet-5'].includes(id), talos: id => ['gpt-6-luna', 'claude-sonnet-4-6'].includes(id) }
...
test('read: a non-string stored id reads as unset — the consumers resolve through their own registries', async () => {
    _resetHouseModelsCache()
    const f = fakeDb({ _id: DOC_ID, chatModel: 42, talosModel: 'gpt-6-luna' })
    const h = await getHouseModels(f.getDb)
    assert.equal(h.chatModel, null); assert.equal(h.talosModel, 'gpt-6-luna')
})
...
test('write: validates each id with the predicate passed, upserts, stamps who and when, and refreshes the cache', async () => {
    ...
    const out = await setHouseModels({ chatModel: 'gpt-6-luna' }, 'roy', { allowed: ALLOWED, getDb: f.getDb })
    assert.equal(out.chatModel, 'gpt-6-luna'); assert.equal(out.talosModel, null)
    ...
    assert.equal((await getHouseModels(f.getDb)).chatModel, 'gpt-6-luna', 'the next read sees the save at once')
    ...
    assert.deepEqual([f.current().chatModel, f.current().talosModel], ['gpt-6-luna', 'claude-sonnet-4-6'])
})
...
test('write: an unregistered id, a non-string, or an empty body is a 400 and writes nothing', async () => {
    ...
    await assert.rejects(setHouseModels({ chatModel: 'gpt-6-luna' }, 'roy', { getDb: f.getDb }), err => err.status === 400)
...
test('the ceiling degrades only onto something cheaper than what would have run', () => {
    ...
    assert.equal(costlierThanCheap('gpt-6-luna'), false, 'Luna is a fifth of Haiku — "degrading" onto Haiku would cost more')
