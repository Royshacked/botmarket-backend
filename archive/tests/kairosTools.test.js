...
import { TRADING_TOOLS, buildTradingToolHandlers } from '../../services/tools/trading.tools.js'
...
test('get_correlations tool is registered with a tickers-array schema', () => {
    const tool = TRADING_TOOLS.find(t => t.name === 'get_correlations')
    assert.ok(tool, 'get_correlations must be in TRADING_TOOLS')
...
test('get_correlations has a callable handler', () => {
    const handlers = buildTradingToolHandlers(null)
...
test('every non-native tool has exactly one handler, and vice versa', () => {
    const handlers   = buildTradingToolHandlers(null)
    const toolNames  = TRADING_TOOLS.map(t => t.name).filter(n => !NATIVE_TOOLS.has(n))
    ...
    for (const key of handlerKeys) {
        assert.ok(
            TRADING_TOOLS.some(t => t.name === key),
