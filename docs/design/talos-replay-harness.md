...
joined the chat menu the same day as fit data points (Roy: "why not put the others"); Mistral moved
from its own API to OpenRouter later that day, for the web plugin (same price). First Mentor turn on Luna (2026-09-20, NVDA framing): six tools in one
