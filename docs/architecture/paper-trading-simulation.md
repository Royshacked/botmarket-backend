...
- **Monitor poll:** paper fills are driven by the monitors — Talos (`setup`) and the kind-blind
  entry/exit loops — while `paperFillService` / `paperMarkService` run on their own loops regardless.
