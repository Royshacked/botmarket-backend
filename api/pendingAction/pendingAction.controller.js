 ...
 */
export const getPendingActions = handle('getPendingActions', async (req, res) => {
 ...
 *
 * The inner try/catch is the UNWIND, not the answer: a throw after the claim puts the row back and
 * is rethrown, so the wrapper logs it and the client gets the one 500 shape.
 */
export const executePendingAction = handle('executePendingAction', async (req, res) => {
    const { id } = req.params
 ...
 */
export const cancelPendingAction = handle('cancelPendingAction', async (req, res) => {
    const { id } = req.params
    const record = await getById(id, req.user._id)
    if (!record) throw httpError(404, 'Not found')
